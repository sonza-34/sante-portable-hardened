...
<form method="POST" action="{{ route('medical-record.update') }}" class="card">
    @csrf
    @method('PUT')
    ...
    <div class="grid md:grid-cols-3 gap-4 mb-6">
        <div>
            ...
            <input type="number" step="0.1" name="height_cm" class="input" value="{{ $record->height_cm }}">
        </div>
        <div>
            <label class="label">Poids (kg)</label>
            <input type="number" step="0.1" name="weight_kg" class="input" value="{{ $record->weight_kg }}">
        </div>
        <div>
            <label class="label">Groupe sanguin</label>
            <input type="text" name="blood_type_patient" class="input"
                   value="{{ $user->blood_type }}" disabled>
            <input type="hidden" name="blood_type" value="{{ $user->blood_type }}">
    ...
    <div class="space-y-4 mb-6">
        <div>
            ...
            <textarea name="chronic_conditions" class="input" rows="2"
                placeholder="Diabète, hypertension, asthme...">{{ $record->chronic_conditions }}</textarea>
        </div>
        <div>
            <label class="label">Allergies</label>
            <textarea name="allergies" class="input" rows="2"
                placeholder="Pénicilline, pollen, fruits de mer...">{{ $record->allergies }}</textarea>
        </div>
        <div>
            <label class="label">Traitements en cours</label>
            <textarea name="current_treatments" class="input" rows="2"
                placeholder="Metformine 500mg 2x/jour...">{{ $record->current_treatments }}</textarea>
        </div>
        <div>
            <label class="label">Antécédents familiaux</label>
            <textarea name="family_history" class="input" rows="2">{{ $record->family_history }}</textarea>
        </div>
        <div>
            <label class="label">Antécédents chirurgicaux</label>
            <textarea name="surgical_history" class="input" rows="2">{{ $record->surgical_history }}</textarea>
        </div>
    </div>
